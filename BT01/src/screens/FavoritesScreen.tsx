...
import { useGetMyFavoritesQuery } from '../services/api/favoriteApi';
import { appTheme } from '../theme/appTheme';

type Props = NativeStackScreenProps<RootStackParamList, 'Favorites'>;

export default function FavoritesScreen({ navigation }: Props) {
  const { data, isLoading, refetch, isFetching } = useGetMyFavoritesQuery();
    ...
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.center}>
              <Text style={{ color: appTheme.colors.textMuted }}>Chưa có sản phẩm yêu thích.</Text>
            </View>
          }
          renderItem={({ item }) => {
